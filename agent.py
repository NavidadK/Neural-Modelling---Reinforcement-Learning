...
class DynaAgent(Environment):
    ...
    def __init__(self, alpha, gamma, epsilon, epsilon_greedy=False):
    ...
    def _init_experience_buffer(self):
        ...
        self.experience_buffer = np.zeros((self.num_states*self.num_actions, 4), dtype=int) # experiences for each combination of state and action
        for s in range(self.num_states):
            for a in range(self.num_actions):
                self.experience_buffer[s*self.num_actions+a] = [s, a, 0, s] # [state, action, reward, next state] 
    ...
    def _update_experience_buffer(self, s, a, r, s1):
        ...
        #self.experience_buffer[index] = [s, a, r, s1]

        # Check if combination already exists
        if self.experience_buffer[index][0] == s and self.experience_buffer[index][1] == a:
            # Update the existing entry with the new reward and next state
            self.experience_buffer[index][2] = r
            self.experience_buffer[index][3] = s1
        else:
            # create new entry if combination does't exisst
            self.experience_buffer[index] = [s, a, r, s1]
    ...
    def _update_qvals(self, s, a, r, s1, bonus=False):
        ...
        if bonus:
            self.Q[s,a] = self.Q[s,a] + self.alpha * (r + (self.epsilon * np.sqrt(self.action_count[s,a])) + self.gamma * self.Q[s1, np.argmax(self.Q[s1, :])] - self.Q[s,a])
        else:
            self.Q[s,a] = self.Q[s,a] + self.alpha * (r + self.gamma * self.Q[s1, np.argmax(self.Q[s1, :])] - self.Q[s,a])
    ...
    def _plan(self, num_planning_updates):
        ...
        for _ in range(num_planning_updates):
            # Sample a random experience from the experience buffer
            sample_index = np.random.randint(0, len(self.experience_buffer))
            experience = self.experience_buffer[sample_index]

            # Extract components of the experience
            s, a, r, s1 = experience

            # Update Q-values using the sampled experience
...
class TwoStepAgent:
    ...
    def simulate(self, num_trials):
        ...
        # Initialize reward
        r_probs = reward_probs = np.ones((2,2)) * 0.5

        # transition probability
        prob_transition = np.zeros((2,2))
        ...
        for _ in range(num_trials):
            ...
            # first stage
            # Computing Q_net
            Q_net[0,:] = self.w * Q_TD[0,:] + (1-self.w) * Q_MB[0,:]

            # Softmax transformation
            prob_s1 = np.exp(self.beta1 * (Q_net[0,:] + self.p * rep)) / np.sum(np.exp(self.beta1 * (Q_net[0,:] + self.p * rep)))
            # choosing action
            a1 = np.random.choice([2,3], p=prob_s1)
            # common state or rare state
            ...
            # second stage
            if((a1 == 2 and common) or (a1 == 3 and not common)):
                idx_s2 = 2
            else:
                idx_s2 = 3

            '''
            Could not finish the code / get it to work ...
            '''
